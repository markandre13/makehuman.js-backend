...
void wsInit() {
    ...
    if (sfd == -1) {
        cerr << "Failed to create server socket 9001" << std::endl;
        exit(EXIT_FAILURE);
    }
...
void wsHandle(bool block) {
    ...
    while (p != handlers.end()) {
        auto fail = false;
        if (FD_ISSET((*p)->fd(), &rd)) {
            // std::cout << (*p)->name() << " can read data on socket " <<
            // (*p)->fd() << std::endl;
        ...
        if (FD_ISSET((*p)->fd(), &wr)) {
            // std::cout << "  can write data on socket " << (*p)->fd() <<
            // std::endl;
        ...
        if (FD_ISSET((*p)->fd(), &ex)) {
            // std::cout << (*p)->name() << " exception on socket " << (*p)->fd() <<
            // std::endl;
            fail = true;
        }
        ...
        if (next) {
            // std::cout << (*p)->name() << " span next handler on socket " <<
            // (*p)->fd() << std::endl;
            handlers.insert(next);
        }

        if ((*p)->finish()) {
            // std::cout << (*p)->name() << " close handler for socket " <<
            // (*p)->fd() << std::endl;
            delete *p;
            p = handlers.erase(p);
            continue;
