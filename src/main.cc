...
// this is how the stub would look like generated by the idl compiler
...
class Backend_impl : public Backend_skel {
    ...
    protected:
        ...
        uint16_t methodB() {
            if (!client) {
                cerr << "Backend_impl::methodB(): client == nullptr" << endl;
                return 0;
            }
        ...
        void _call(const std::string_view &operation, CORBA::GIOPDecoder &decoder, CORBA::GIOPEncoder &encoder) override {
            println("Backend_impl::call(\"{}\")", operation);
            if (operation == "call") {
                encoder.buffer.ushort(call());
                return;
            }
            if (operation == "setClient") {
                cerr << "========================================================================= enter setClient()" << endl;
                ...
                // client hasn't been implemented yet, so we fake it
                auto stub = make_shared<Client_stub>(orb, move(objectRef));
                // auto stub = Client::_narrow(objectRef);
                setClient(stub);
                cerr << "========================================================================= leave setClient()" << endl;
                return;
            }
